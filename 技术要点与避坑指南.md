...
### 4. GitHub 发布与网络
- **HTTP/1.1 配置**：在国内网络环境下，`git push` 或 `gh release create` 常因 HTTP/2 协议问题中断。
    - **症状**：`RPC failed; curl 92 HTTP/2 stream 0 was not closed cleanly`
    - **解决**：强制使用 HTTP/1.1
      ```bash
      git config --global http.version HTTP/1.1
      ```
- **大文件限制**：GitHub 对单个文件大小有限制（通常 100MB），虽然本工具体积很小，但若包含大量资源文件，务必注意。
